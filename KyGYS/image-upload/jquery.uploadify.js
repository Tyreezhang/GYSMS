		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					successTimeout  : 40,                 // The number of seconds to wait for Flash to detect the server's response after the file has finished uploading
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
						    this.queueData.errorMsg += '\n当前待上传序列中最多5张,请先上传后再添加.';
						} else {
							this.queueData.errorMsg += '\n单次最多只能上传5张.';
						}
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n"' + file.name + '"文件大小超出限制 应为 limit (' + settings.fileSizeLimit + ')M.';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\nThe file "' + file.name + '" 文件大小为0.';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
